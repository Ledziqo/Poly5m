...
export default function NewsFeed({ headlines, sentiment }: { headlines: string[], sentiment: any }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        {sentiment && (
          <span className={`text-xs font-medium px-2 py-1 rounded border ${sentiment.score > 0 ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-pink-500/10 text-pink-400 border-pink-500/20'}`}>
      ...
      {sentiment?.reasoning && (
        <div className="mt-4 p-3 bg-purple-500/5 text-purple-300 text-xs rounded-lg border border-purple-500/20">
          <strong className="text-purple-400">AI Analysis:</strong> {sentiment.reasoning}
        </div>
      )}
    </motion.div>
  );
